///MLP_AddBinaryOper(operator, precedence, script, returntype, lhstype, rhstype[, assoc])
...
**      assoc       associtivity of operator (default is right/MLP_O_LEFTASSOC
**
**  Returns:
...
if (ds_map_exists(P_BINOPER,argument[0])) {
    ...
    with (o) {
        ds_map_add(ActualFunctions, argument[4] +"$" + argument[5], 
                    _MLP_AddBinarySig(argument[2], argument[3], argument[4], argument[5]));
    }
} else {
    var assoc;
    assoc = MLP_O_LEFTASSOC;
    if (argument_count > 6) assoc = argument[6];
    o = _MLP_NewBinaryOper(argument[0], argument[1], argument[2], argument[3], argument[4], argument[5], assoc);
