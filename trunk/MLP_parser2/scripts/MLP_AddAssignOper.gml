///MLP_AddAssignOper(operator, precedence, script, returntype, lhstype, rhstype[, assoc])
...
if (ds_map_exists(P_ASSIGNOPER,argument0)) {
    ...
    with (o) {
        ds_map_add(ActualFunctions, argument4 +"$" + argument5, 
                    _MLP_AddAssignSig(argument2, argument3, argument4, argument5));
    }
} else {
    var assoc;
    assoc = MLP_O_RIGHTASSOC;
    if (argument_count > 6) assoc = argument[6];
    o = _MLP_NewAssignOper(argument0, argument1, argument2, argument3, argument4, argument5, assoc);
