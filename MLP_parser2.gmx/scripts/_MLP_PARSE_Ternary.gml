...
if (ds_stack_size(argument1) < 3) {
    MLP_RaiseException(MLP_EXCEPT_TERNARY,argument0.tokenpos,
...
if (mhs.tokentype == MLP_TT_VALUE) {
    if (mhs.operator == MLP_VAL_REAL) {
        mhs_type = MLP_VAL_REAL;
        mhs_val = real(mhs.tokenstring);
    } else {
        mhs_type = MLP_VAL_STRING;
